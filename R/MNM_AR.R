...
#'
#' @description Fits a multi-species N-mixture (MNM) model with an autoregressive component (AR-1) using Nimble.
...
#' @details The model incorporates temporal and species-level covariates and accounts for serial correlation in abundance using an AR(1) process.
...
#' @param iterations Integer. Number of MCMC iterations to run. Defaults to 60,000.
#' @param burnin Integer. Number of iterations to discard as burn-in. Defaults to 20,000.
#' @param thin Integer. Thinning interval for saving MCMC samples. Defaults to 10.
...
print(paste0("Observations entered correspond to ", R, " sites, ", T, " sampling occasions, ", S, " species and ", K, " timepoints. If this appears to be incorrect, please re-format your data into an array of dimension (R,T,S,K)."))
...
if(!is.null(Xn) & length(dim(Xn))==length(dim(Y))){
  ...
  # Run the monitor function to obtain rhat values
  ...
  mnm_object <- new("MNM",
                    ...
                    n_parameters=3*S+(R*S)+((S*(S+1))/2)+2+(dim_Xp*S)+(dim_Xn*S), # calculate number of parameters, for use in AIC/BIC calculation
